...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
      ...
      // System.out.println(String.format("My Rank: %d, Size: %d, Row Chunk: %d, Start: %d, End: %d",
      //        myRank, size, rowChunk, startRow, endRow));
        ...
        for (int i = startRow; i < endRow; i++) {
            int aOffset = a.getOffsetOfRow(i);
            for (int j = 0; j < c.getNCols(); j++) {
                int cIndex = c.getOffsetOfRow(i) + j;
                cValues[cIndex] = 0.0;
                for (int k = 0; k < b.getNRows(); k++) {
                    c.incr(i, j, aValues[aOffset + k] * b.get(k, j));
                }
            }
        }
        ...
        if (myRank == 0) {
            ...
            // System.out.println("Rank 0 Waiting");
            mpi.MPI_Waitall(requests);
            // System.out.println("Rank 0 DONE");
        } else {
            //System.out.println(String.format("Rank: %d sending data", myRank));
